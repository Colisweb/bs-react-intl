...
let extractMessagesFromValueBindings = (callback, _iterator, valueBindings: list(value_binding)) =>
  ...
  |> List.map(valueBinding =>
       ...
       | {pvb_pat: {ppat_desc: Ppat_var(_)} as pattern, pvb_expr: {pexp_desc: Pexp_record(fields, None)} as expr} =>
         Ast_helper.Vb.mk(
           ~attrs=valueBinding.pvb_attributes,
           ~loc=valueBinding.pvb_loc,
           pattern,
...
let getIterator = callback => {
  ...
  expr: (iterator, expr) => {
    ...
    | {pexp_desc: Pexp_apply({pexp_desc: Pexp_ident({txt, _})} as applyExpr, labels)}
